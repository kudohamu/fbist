fvistFoundationApp.controller("friendsSummaryController", ["$scope", "$resource", "recordFactory", "gundamsFactory", "friendsFactory", "recordTotalFactory", "friendsSummaryFactory", "selectGundamFactory", "selectFriendFactroy", "messageFactory", function($scope, $resource, recordFactory, gundamsFactory, friendsFactory, recordTotalFactory, friendsSummaryFactory, selectGundamFactory, selectFriendFactroy, messageFactory) {
  //初期化
  ...
  //並び替え関連
  $scope.sort = { id: true, total: false, won: false, lost: false, rate: false };
  $scope.asc = true;
  $scope.reverse = false;
  $scope.predicate = "id";
  ...
  function getSummary() {
    ...
    $scope.records = friendsSummaryFactory.query({ section: $scope.filter_section, team: $scope.filter_team, match: $scope.filter_match }, function() {
